...
public class MOJacksonYB extends HttpSite {
	...
	public LoginResponse onLogin() {
		try {
			...
			if (resp.getResponseAsString().contains("Title Search - Special Assessments")) {

				FormTag f = getFormFromResponse("form1", resp);
	...
	private FormTag getFormFromResponse(String name, HTTPResponse resp) {
		...
		if (StringUtils.isNotEmpty(respString)) {
			...
			try {
				nodes = nodes.extractAllNodesThatMatch(new TagNameFilter("form"), true).extractAllNodesThatMatch(new HasAttributeFilter("id", name));
				if (nodes.size() > 0) {
