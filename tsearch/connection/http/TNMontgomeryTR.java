...
public class TNMontgomeryTR extends HTTPSite 
{
    ...
    {        
        setUserData(InstanceManager.getManager().getCurrentInstance(searchId).getCrtSearchContext().getSearchID() + "TNMontgomeryTR");
        
        if ("".equals(sid)) {
        
        	HTTPRequest req = new HTTPRequest( "http://www.montgomery.tennesseetrustee.org" );
    ...
    public void onBeforeRequest(HTTPRequest req) 
    {
        ...
        if ( url.indexOf("search.php") != -1 && url.indexOf("owner=") != -1 ) {
        	...
        	loggingIn = true;
        	HTTPRequest first = new HTTPRequest( "http://www.montgomery.tennesseetrustee.org" );
//        	first.setHeader( "Cookie", sid );
            process( first );
            
            req.setHeader("Host", "www.montgomery.tennesseetrustee.org");                    
            req.setHeader("Referer", "http://www.montgomery.tennesseetrustee.org");
