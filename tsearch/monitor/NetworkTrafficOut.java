...
public class NetworkTrafficOut extends AgentObject
{
    /*
     * SNMP agent for network monitoring on SunOS
     */
	private static final Logger logger = Logger.getLogger(NetworkTrafficOut.class);
	private long previous_received_traffic = -1;
	private String currentOID;
	
    public NetworkTrafficOut(String oid)
    {
        super(oid);
        currentOID = oid;
    }
    public VarBind get(String oid) throws MessageException
    {
        ...
        logger.info( oid + " [START] NetworkTrafficOut = " + last_network_traffic );
        
        //if OS is Solaris, get the network stats
        if( System.getProperty("os.name").startsWith("SunOS") )
        {
            last_network_traffic = getOutboundTraffic();
        }
        ...
        //traffic is the last 5 minute average --> divide with 300 (5 minutes) if not localhost traffic
        if( currentOID.equals( SNMPAgent.OUT_NETWORK_TRAFFIC ) )
        {
            traffic = traffic / 300;
        }

        logger.info( oid + " [END] NetworkTrafficOut = " + last_network_traffic + " - " + previous_received_traffic + "=" + traffic );
        return new VarBind(new String(getOID()), new Unsigned(traffic));
    }
    
    private long getOutboundTraffic()
    {
        long return_value = 0;
        ...
        try
        {
	        ...
	        while( tokenizer.hasMoreTokens() )
	        {
	            ...
	            while ( tokenizer2.hasMoreTokens() )
	            {
	                ...
	                if( attrName.equals( "obytes" ) && currentOID.equals( SNMPAgent.OUT_NETWORK_TRAFFIC ) )
	                {
	                    return_value = Long.parseLong( value );
		                //traffic in bytes
		                return return_value ;
	                }
	                else if(attrName.equals( "opackets" ) && currentOID.equals( SNMPAgent.OUT_LOCALHOST_TRAFFIC ))
	                {
	                    return_value = Long.parseLong( value );
		                //traffic in bytes
		                return return_value ;	                    
