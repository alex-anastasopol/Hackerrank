...
public class FLGenericDTG extends TSServerDTG implements DTLikeAutomatic {

	protected static final String FL_DTG_FAKE_RESPONSE = ro.cst.tsearch.utils.StringUtils.fileReadToString(TSServerDASL.FAKE_FOLDER + "FL_DTG_FakeResponse.xml");
	protected static final String[] PLAT_CONDO = {"PLCU", "PLAM"};
	
	private static final long serialVersionUID = -39475262378410L;

	public FLGenericDTG(String rsRequestSolverName, String rsSitePath, String rsServerID, String rsPrmNameLink, long searchId, int mid)
			throws FileNotFoundException, IOException {
		super(rsRequestSolverName, rsSitePath, rsServerID, rsPrmNameLink, searchId, mid);
		resultType = MULTIPLE_RESULT_TYPE;
	}

	public FLGenericDTG(long searchId) {
		super(searchId);
	}
		...
		if (global.getSearchType() == Search.AUTOMATIC_SEARCH) {
			...
			if ("FL".equals(state)){
				...
				try {
					long siteId = -1;
					try {
						siteId = TSServersFactory.getSiteId(state, county, "DT");
					} catch (Exception e) {
					}
					if (siteId > 0){
						DataSite data =  HashCountyToIndex.getDateSiteForMIServerID( 
			        			InstanceManager.getManager().getCommunityId(searchId), (int) siteId);
						
						if (data != null){
							tsInterface = TSServersFactory.GetServerInstance((int) siteId, searchId);
						}
					}
				} catch (Exception e) {
					e.printStackTrace();
				...
				DTLikeAutomatic dtAuto = null;
				
				if (tsInterface instanceof DTLikeAutomatic){
					dtAuto = (DTLikeAutomatic) tsInterface;
				}
				if (dtAuto != null){
					...
					module.clearSaKeys();
					PlatIterator it = getPlatBookPageIterator(searchId, dtAuto);
					...
					module.clearSaKeys();
					it = getPlatBookPageIterator(searchId, (DTLikeAutomatic) dtAuto);
					module.addIterator(it);
					if (!isUpdate) {
						modules.add(module);
					}
					...
					module.clearSaKeys();
					it = getPlatBookPageIterator(searchId, dtAuto);
					module.addIterator(it);
					if (!isUpdate) {
						modules.add(module);
					}
	...
	protected String prepareInstNoForReferenceSearch(InstrumentI inst) {
		String instrNumber = inst.getInstno();
		if (CountyConstants.FL_Broward == dataSite.getCountyId() && instrNumber.length() > 8){
			return instrNumber;
		}
	...
	protected String prepareInstrumentYearForReferenceSearch(InstrumentI inst) {
		...
		if (CountyConstants.FL_Broward == dataSite.getCountyId() && inst.getInstno().length() > 8){
			return "";
		}
	...
	class LegalDescriptionIterator extends GenericRuntimeIterator<PersonalDataStruct> implements LegalDescriptionIteratorI{
			...
			protected List<PersonalDataStruct> createDerrivations() {
				...
				List<PersonalDataStruct> legalStructList = (List<PersonalDataStruct>) global.getAdditionalInfo(AdditionalInfoKeys.DG_LOOK_UP_DATA);
				...
				platPageAO = platPageAO.replaceFirst("^(\\d+)-\\d+$", "$1");
				...
				if (legalStructList == null) {
					...
					try {
						m.getAccess();
						List<RegisterDocumentI> listRodocs = FLSubdividedBasedDASLDT.getGoodDocumentsOrForCurrentOwner(m, global, true, GWTDataSite.DG_TYPE, StateContants.STATE_ABBREV.FL.toString());

						if (listRodocs == null || listRodocs.size() == 0) {
							listRodocs = FLSubdividedBasedDASLDT.getGoodDocumentsOrForCurrentOwner(m, global, false, GWTDataSite.DG_TYPE, StateContants.STATE_ABBREV.FL.toString());
						}
				...
				switch(module.getModuleIdx()){
					...
					case TSServerInfo.ARB_MODULE_IDX:{
						if(org.apache.commons.lang.StringUtils.isNotBlank(str.arbBook)){
							...
							module.setData(1, str.getTownship());
							module.setData(2, str.getRange());
							module.setData(3, str.quarterValue);
		...
		InstrumentGenericIterator instrumentGenericIterator = new InstrumentGenericIterator(searchId, dataSite) {
			...
			protected String cleanInstrumentNo(String instno, int year) {
				if (StringUtils.isNotEmpty(instno)){
					if (instno.length() <= 8) {
						...
						if (CountyConstants.FL_Broward == dataSite.getCountyId() && instno.length() > 8){
							return instno;
						} else if (CountyConstants.FL_Lee == dataSite.getCountyId() && instno.length() > 11){
							instno = instno.substring(4);
							return StringUtils.stripStart(instno, "0");
						} else{
							return instno.substring(instno.length() - 8).replaceFirst("^0+", "");
						}
					}
				}
				return "";
			...
			public String getYearFrom(InstrumentI state, HashMap<String, String> filterCriteria) {
				if (state.getYear() != SimpleChapterUtils.UNDEFINED_YEAR) {
					if (CountyConstants.FL_Broward == dataSite.getCountyId() && state.getInstno().length() > 8){
						return "";
					} else if (CountyConstants.FL_Lee == dataSite.getCountyId() && state.getInstno().length() > 11){
						if (state.getYear() > SimpleChapterUtils.UNDEFINED_YEAR){
							return Integer.toString(state.getYear());
						} else{
							return state.getInstno().substring(0,4);
						}
					} else{
						return Integer.toString(state.getYear());
					}
...
//		}
	...
	protected DownloadImageResult saveImage(ImageI image)throws ServerResponseException {
		...
		if (StringUtils.isEmpty(instrDesc)){
    		if (StringUtils.isNotEmpty(docNumber) && StringUtils.isNotEmpty(year)){
    			instrDesc = year + "." + docNumber;
    		}
    	} else if (!instrDesc.contains(".") && StringUtils.isNotEmpty(year)){
    		instrDesc = year + "." + instrDesc;
    	}
    	
    	instrDesc = getInstrumentNumberForRO2ImageSearch(instrDesc, yearInt, monthInt);
		...
		//if (!isFake){
			if (!savedImage){
				if (StringUtils.isNotEmpty(instrDesc)){
					FLGenericMFCServerRO2 mfcRO = isCountyAvailableOnMyFlorida();
					
					if (mfcRO != null && mfcRO.getDataSite().isEnableSite(getCommunityId())){
						try{
							savedImage = mfcRO.downloadImageFromMyFlorida(i, image, instrDesc, null);
							if (savedImage){
								SearchLogger.info("<br/>Image(searchId=" + searchId + ") for book=" + i.getBook() + " page=" + i.getPage() + "; inst=" + instrDesc 
										+ " year=" + year + " was taken from RO2.<br/>", searchId);
							}
						} catch(Exception e) {
							e.printStackTrace();
						}
					} else{
						SearchLogger.info("<br/>Image(searchId=" + searchId + " )book=" + i.getBook() + "page=" + i.getPage() + "inst=" + instrDesc 
								+ " can't be taken from RO2 because this county is unavailable on this site.<br/>", searchId);
					}
				}
			}
	...
	protected String getInstrumentNumberForRO2ImageSearch(String instrDesc, int yearInt, int monthInt) {
	
		return FLGenericMFCServerRO2.prepareInstrumentNumberForRO2(instrDesc, yearInt, monthInt, dataSite.getCountyIdAsString());
	}	
	...
	public HashMap<String, String> makeInstrumentSearchOnOtherServer(InstrumentI instrument){
		...
		if (mfcRO != null && mfcRO.getDataSite().isEnableSite(getCommunityId())){
			return mfcRO.getDocumentFromRO2(instrument);
		}
		
		return new HashMap<String, String>();
	}
	
	public FLGenericMFCServerRO2 isCountyAvailableOnMyFlorida(){
		
		FLGenericMFCServerRO2 mfcRO = null;
		try {
			...
			if (data != null){
				mfcRO = (FLGenericMFCServerRO2) TSServersFactory.GetServerInstance((int) siteId, getSearch().getID());
			}
		} catch (Exception e) {
			logger.error("RO2 is not available for county:" + dataSite.getCountyName() + "; searchid=" + searchId);
		}
	...
	protected void addDefaultPlatMapSearch(TSServerInfoModule module,PersonalDataStruct str, String type) {
		...
		if( StringUtils.isEmpty(book)||StringUtils.isEmpty(page) ){
			try{
				m.getAccess();
				String[] subdivVector = FLSubdividedBasedDASLDT.getSubdivisionVector(m);
					...
					for (DocumentI ro2Document : ro2Documents) {
						...
						if (ro2Document != null) {
							...
							if (StringUtils.isNotEmpty(ro2DocInstNo) && StringUtils.isNotEmpty(year)) {
								...
								isAlreadySaved = docClone.flexibleEquals(ro2DocumentClone);
								if (isAlreadySaved) {
									break;
								}
							}
