 ...
 */
public class FLPalmBeachDT extends FLSubdividedBasedDASLDT{
	private static final long serialVersionUID = 3477834L;

	public FLPalmBeachDT(long searchId) {
		super(searchId);
	}
	
	public FLPalmBeachDT(String rsRequestSolverName, String rsSitePath, String rsServerID, String rsPrmNameLink, long searchId, int mid) {
	...
	public void addAssesorMapSearch(TSServerInfoModule module, PersonalDataStruct str) {
		...
		if(!StringUtils.isEmpty(apn) && StringUtils.isEmpty(tw)){
			...
			if(apn.length()>=8){
				rg = apn.substring(0,4);
				tw = apn.substring(4,6);
				sec = apn.substring(6,8);
			}
		}
