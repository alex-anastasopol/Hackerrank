 ...
 * @author MihaiB
 */
public class MIGenericDTG extends TSServerDTG implements DTLikeAutomatic{

	private static final long serialVersionUID = 3024488184071095461L;
	private static final String MULTIPLE_LEGAL = "MI_MULTIPLE_LEGAL_INSTR";

	public MIGenericDTG(String rsRequestSolverName, String rsSitePath, String rsServerID, String rsPrmNameLink, long searchId, int mid)
			throws FileNotFoundException, IOException {
		super(rsRequestSolverName, rsSitePath, rsServerID, rsPrmNameLink, searchId, mid);
	}

	public MIGenericDTG(long searchId) {
		super(searchId);
	}
...
//						}
	...
	public void addPlatMapSearch(TSServerInfoModule module,	LegalStructDTG str) { 
		addDefaultPlatMapSearch(module, str, PLAT_MAP_SUBDOCTYPE);
	}
	
	protected void addDefaultPlatMapSearch(TSServerInfoModule module, LegalStructDTG str, String type) {
				...
				for (DocumentI doc : list){
					for (PropertyI prop : doc.getProperties()){
						if (prop.hasSubdividedLegal()){
							...
							if (StringUtils.isNotEmpty(book1) && StringUtils.isNotEmpty(page1)){
								book = book1;
								page = page1;
								found = true;
								break;
		...
		if (StringUtils.isNotEmpty(book) && book.length() >= 2){
			book = book.replaceFirst("[a-zA-Z]", "");
		}
		
		if (StringUtils.isNotEmpty(page) && page.length() >= 2){
		...
		module.forceValue(4, PLAT_DOCTYPE);
		module.forceValue(5, type);
		return;
	}
	
	public void addAssesorMapSearch(TSServerInfoModule module, LegalStructDTG str, String additionalForSection) {
		CurrentInstance ci 	= InstanceManager.getManager().getCurrentInstance(searchId);
		Search search 		= ci.getCrtSearchContext();
		...
		if (StringUtils.isNotEmpty(tw)){
			tw = tw.replaceAll("[a-zA-Z]", "");
			tw = tw.replaceFirst("^0+", "");
		}
		
		if (StringUtils.isNotEmpty(rg) && rg.length()>2){
			rg = rg.replaceAll("[a-zA-Z]", "");
			rg = rg.replaceFirst("^0+", "");
		}
		
		if (StringUtils.isNotEmpty(sec) && sec.length()>2){
		...
		module.forceValue(4, PLAT_DOCTYPE);
		module.forceValue(5, ASSESSOR_MAP_SUBDOCTYPE);
	...
	class LegalDescriptionIterator extends GenericRuntimeIterator<LegalStructDTG> implements LegalDescriptionIteratorI{
			...
			protected List<LegalStructDTG> createDerrivations() {
				...
				if (legalStructList == null) {
					...
					try {
						m.getAccess();
						List<RegisterDocumentI> listRodocs = getGoodDocumentsOrForCurrentOwner(m, global, true, GWTDataSite.DG_TYPE, StateContants.STATE_ABBREV.MI.toString());

						if (listRodocs == null || listRodocs.size() == 0) {
							listRodocs = getGoodDocumentsOrForCurrentOwner(m, global, false, GWTDataSite.DG_TYPE, StateContants.STATE_ABBREV.MI.toString());
						}
				...
				switch(module.getModuleIdx()){
					case TSServerInfo.SUBDIVISION_MODULE_IDX:
						...
						if (StringUtils.isNotEmpty(str.getLotThru())){
							module.setData(3, str.getLotThru());
						}
						module.setData(4, str.getBlock());
						module.forceValue(5, str.getSubLot());
						module.setData(10, str.getPlatBook());
						module.setData(11, str.getPlatPage());
...
//			InstrumentI instr = clone.getInstrument();
	...
	public static String prepareApnPerCounty(DataSite dataSite, String originalApn){
		...
		switch (dataSite.getCountyId()) {
			case CountyConstants.MI_Macomb:
			case CountyConstants.MI_Oakland:
				apn = apn.replaceAll("-", "");
				if (apn.length() == 12){
					apn = apn.replaceAll("(\\d{2})(\\d{2})(\\d{3})(\\d{3})", "$1-$2-$3-$4");
					originalApn = apn;
				}
				break;
			default:
