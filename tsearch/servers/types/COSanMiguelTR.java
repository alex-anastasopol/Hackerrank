...
public class COSanMiguelTR extends COGenericTylerTechTR {

	/**
	 * @author mihaiB
	 */
	private static final long serialVersionUID = 7992302989861587704L;

	public COSanMiguelTR(String rsRequestSolverName, String rsSitePath,
	...
	protected String clean(String result) {
		...
		result = result.replaceAll("(?is)<p[^>]*>The amounts of taxes due on this page are based on <strong>last year's</strong> property value assessments\\..*?</p>", "");
		return result;
	}
	...
	protected void setModulesForAutoSearch(TSServerInfo serverInfo) {
		...
		if(hasStreet()){
			TSServerInfoModule module = new TSServerInfoModule(serverInfo.getModule(TSServerInfo.NAME_MODULE_IDX));
			module.clearSaKeys();
			module.getFunction(5).setSaKey(SearchAttributes.P_STREETNO);
			module.getFunction(7).setSaKey(SearchAttributes.P_STREETNAME);
