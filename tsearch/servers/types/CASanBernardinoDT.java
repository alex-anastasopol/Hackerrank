...
import com.stewart.ats.base.name.NameI;
	...
	/**
	 * @author Cornel Tremurici
	 **/
	public class CASanBernardinoDT extends CAGenericDASLDT {

		private static final long serialVersionUID = -7721629532980516160L;
		...
		public TSServerInfo getDefaultServerInfo() {
			...
			if ( !StringUtils.isEmpty(pin) ) {
				...
				if ( pinSize >= 10 && !pin.contains("-") ) {
					...
					String pin1 = pin.substring(0, 4); // split in 3 - 3 - 3 format 1043-421-12-0-000
					String pin2 = pin.substring(4, 7);
					String pin3 = pin.substring(7, 9);
					String pin4 = pin.substring(9, 10);
					String pin5 = pin.substring(10);

					String newpin = pin1+"-"+pin2+"-"+pin3+"-"+pin4+"-"+pin5;
		...
		public CASanBernardinoDT(long searchId) {
			super(searchId);
		}

		public CASanBernardinoDT(String rsRequestSolverName, String rsSitePath, String rsServerID, String rsPrmNameLink, long searchId, int mid) {
			super(rsRequestSolverName, rsSitePath, rsServerID, rsPrmNameLink, searchId, mid);
		}
		
		/*modifies doc NO not to start with letters*/
		protected String createLinkForImage(HashMap<String, String> value) {
			...
			build.append("docno");
			build.append("=");
			docno = docno.replaceAll("\\s", "").replaceFirst("^[^\\d]+", "");
			build.append(docno);
			build.append("&");
		...
		protected String modifyXMLResponse(String xml, int moduleIDX) {
			String ret = super.modifyXMLResponse(xml, moduleIDX) ;
			if( !(ret.indexOf("GENERAL_INDEX")>0) ){
				ret = ret.replaceAll(
						"<Book>[^<]*</Book>\\s*<Page>[^<]*</Page>\\s*<Type>([^<]*)</Type>\\s*<RecordedDate>\\s*<Date>([^<]*)</Date>\\s*</RecordedDate>\\s*<DocumentNumber>([^<]+)</DocumentNumber>", 
						"<Book></Book><Page></Page><Type>$1</Type><RecordedDate><Date>$2</Date></RecordedDate><DocumentNumber>$3</DocumentNumber>"
					);
			}
			return ret ;
		}
		...
		protected void setModulesForAutoSearch(TSServerInfo serverInfo) {
			...
			pin=pin.replaceAll("-", "");
			if( pin.length()>=7 ){
				asseorBook = pin.substring( 0, 4 );
				assesorPage = pin.substring( 4, 6 );
				//search for plat
		...
		protected String getInstrFromXML(Node doc, HashMap<String, String> data) {
			String instr = super.getInstrFromXML(doc, data);
			if(instr!=null && !instr.contains("_")){
