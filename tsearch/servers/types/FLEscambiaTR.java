...
public class FLEscambiaTR extends FLGenericGovernmaxTR {
	...
	private static final CheckTangible CHECK_TANGIBLE = new CheckTangible() {
		public boolean isTangible(String row){
			...
			return searchdat5.matches(".{1,12}");
		}
	};
	
	public FLEscambiaTR(String rsRequestSolverName, String rsSitePath, String rsServerID,
	...
	protected void setModulesForAutoSearch(TSServerInfo serverInfo) {
		...
		FilterResponse namefilter = NameFilterFactory.getDefaultNameFilterNoSinonims(
		...
		// P1 : search by PIN with Search by GEO Number module
		if(hasPin()){
		...
		// P1 bis : search by PIN	with Search by Account Number module	
		if(hasPin()){
		...
		if(hasAddress()){
			...
			module.clearSaKeys();
			module.getFunction(0).setSaKey(SearchAttributes.P_STREETNAME);  
			module.addFilter(rejectNonRealEstateFilter);
	...
	public ServerResponse SearchBy(TSServerInfoModule module, Object sd) throws ServerResponseException {
        ...
        if(module.getModuleIdx() == TSServerInfo.PARCEL_ID_MODULE_IDX){
        	...
        	pin = pin.replaceFirst("\\s*(\\d{2})-?(\\d{4})-?(\\d{3})\\s*", "$1-$2-$3");
